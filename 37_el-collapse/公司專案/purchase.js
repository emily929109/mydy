...
const App = {
  setup() {
    ...
    onMounted(() => {
      ...
      // 不論登入與否且網址帶store product時
      if (store && product && store !== "null" && product !== "null") {
    ...
    // =============== 商品頁view 開始 ===============
    const cartQtyForCurrentSpec = computed(() => {
      if (!p.spec) return 0; // 沒有選規格

      const item = productList.value
        .flatMap((dealer) => dealer.cars)
        .find(
          (item) =>
            item.product_id === parseInt(getUrlParameter("product")) &&
            item.item_spec?.name === p.spec.name,
        );
      // console.log(item ?.buy_qty || 0, "item")
      // item_spec ? -> 預防後端傳的資料沒有 item_spec
      return item?.buy_qty || 0;
      // item ? -> 找不到商品時回傳0 (例:刪除時)
    });
    ...
    addToCart = (_p, type) => {
      ...
      // 檢查數量是否超過剩餘庫存
      if (
        buyQty.value + cartQtyForCurrentSpec.value >
        Number(_p.product_qty || 0)
      ) {
        alert("購買數量已超過商品剩餘庫存");
        return;
      }
    ...
    watch(selectedOption, (newVal) => {
      ...
      if (newVal == "商家宅配") {
        ...
        showHomeDiv.value = false;
        productItem.shipping_fee = productItem.tmp_shipping_fee;
      } else if (newVal == "店內自取") {
        showCarDiv.value = false;
        showHomeDiv.value = true;
        productItem.shipping_fee = 0;
      }
    });
    ...
    goToCheckout = (_v) => {
      ...
      axios({
        ...
        headers: { "Content-Type": "application/json" },
        //data: { dealer_id: _v.dealer_id, member_id: member.id, store_no: _store_no, orders: data2 }
        data: { dealer_id: _v.dealer_id, store_no: _store_no, orders: data2 },
      })
        .then((response) => {
          $.unblockUI();
          console.log(response.data);
          if (response.data.success) {
            ...
            productItem.tmp_shipping_fee = response.data.shipping_fee;

            productItem.ishomeDelivery = response.data.ishomeDelivery;
            productItem.isPickUp = response.data.isPickUp;
