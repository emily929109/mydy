...
const menuApp = {
    ...
    setup() {
        ...
        const u = reactive({
            mobile: '', pw: '', code: '', role: 'member',
            mobile2: '', pw2: '', code2: '', role2: 'dealer',
        });
        ...
        showLogin = (member) => {
            if (member == null || member.login_ok_msg != '*') $('#authentication-modal').modal('show');
        };
        ...
        const login = (_mobile, _pw, _role, _login_type) => {
            ...
            axios({
                ...
                params: {
                    ...
                    pw: _pw,
                    role: _role,
                    browser: _getNavigatorVersion() + getAndroidVersion() + '|' + iOSversion(),
                    ...
                    if (response.data.value.login_ok_msg != '*') {
                        ...
                        switch (response.data.value.register_step) {
                            ...
                            case 2:
                                ...
                                if ((url.indexOf('membersign2') >= 0 && url.indexOf('code=' >= 0)) ||
                                    (url.indexOf('membersign2_easy') >= 0 && url.indexOf('code=' >= 0))
                                ) {//轉手機簽署頁
                                    //不顯示註冊未完成
                                    //alert('不顯示註冊未完成');
                                    location.reload();
                        ...
                        if (href == '/home/uploadfile_s' || href == '/home/membersign2_s' || href == '/home/notetime') {
                            return;
                        }
                        //補M1
                        ...
                        if (response.data.value.applyCheckMsg != null) {
                            if (!response.data.value.applyCheckMsg.check) {//過程有錯                               
                                ...
                                switch (response.data.value.applyCheckMsg.type) {
                                    case 1:
                                        ...
                                        alert('您有尚須補送(件)的資料，請確認手機簡訊連結，並在指定時間內完成');
                                        return;
                                    case 2://跳轉註冊流程                                        
                                    ...
                                    case 15:
                                        header_app.Link_Logout();//登出
                                        alert('感謝您的支持，因資料審核未通過，可於半年後再重新申辦DUDUPAY分期，謝謝');
                                        return;
                                    case 17://退件 >= 180天
                                    case 18:
                                    case 19:
                                    ...
                                    case 13:
                                        header_app.Link_Logout();//登出
                                        alert('資料審核中，請先耐心等待簡訊通知。收到通過訊息，即可登入使用！');
                                        return;
                                    case 21://註冊帶訂單 可下單                                      
                        ...
                        var url = window.location.href.toLowerCase();
                        console.log(url);                        
                        if (url.indexOf('order_2') != -1 || url.indexOf('order_3') != -1 || url.indexOf('orderapi') != -1 || url.indexOf('orderpurchase') != -1) location.reload();                        
        ...
        Login2 = (_u) => {
            ...
            axios({
                ...
                headers: { 'Content-Type': 'application/json' },
                params: { mobile: _u.mobile2, pw: _u.pw2, role: _u.role2, browser: _getNavigatorVersion() + getAndroidVersion() + '|' + iOSversion() }
            }).then((response) => {
                $.unblockUI();
                console.log(response.data);
                if (response.data.success) {
        ...
        eye2 = () => {
            ...
            pwFieldType2.value = pwFieldType2.value == 'password' ? 'text' : 'password';
        ...
        nonMemberRegister = async (_buy) => {
            ...
            if (url.indexOf('order_2') != -1 || url.indexOf('order_3') != -1 || url.indexOf('orderapi') != -1) {
                var _data = getQueryParam('data');
                if (_data != 'null') {
                    //解密data get AE86
                    var _type = 0; var _store_id = 0;
                    ...
                    if (res.success) {
                        if (_buy == 1) {//註冊帶訂單
                            var dudu_order = { order_type: '註冊帶訂單', order_url: window.location.href };
        ...
        return {
            u,
            showPassword, pwFieldType, eye, changeCode,
            showPassword2, pwFieldType2, eye2, changeCode2,
            Login, Login2,
            showLogin, register_url, nonMemberRegister,
            sendMobile, unDo, otp_timeout, verify_code, confirmOTP, boboChangePw, login, register_now,
            openRegistModal
