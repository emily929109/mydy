...
function defaultStep2Upload(): Step2Upload {
  return { idFront: null, idBack: null, secondId: null, selfie: null, incomeProof: null }
}
...
export const useRegistrationStore = defineStore('registration', () => {
  ...
  const step1Account = reactive<Step1Account>({ ...defaultStep1Account(), ...persisted.step1Account })
  const step2Upload = reactive<Step2Upload>({ ...defaultStep2Upload(), ...persisted.step2Upload })
  const step3Info = reactive<Step3Info>({ ...persisted.step3Info })
  const step4TwcaConsent = reactive<Step4TwcaConsent>({ ...defaultStep4TwcaConsent(), ...persisted.step4TwcaConsent })
  const step5TwcaClause = reactive<Step5TwcaClause>({ ...defaultStep5TwcaClause(), ...persisted.step5TwcaClause })
  const step6TwcaVerify = reactive<Step6TwcaVerify>({ ...defaultStep6TwcaVerify(), ...persisted.step6TwcaVerify })
  const step7Sign = reactive<Step7Sign>({ ...defaultStep7Sign(), ...persisted.step7Sign })
  ...
  watch(
    [step1Account, step2Upload, step3Info, step4TwcaConsent, step5TwcaClause, step6TwcaVerify, step7Sign, completedSteps],
    persist,
    { deep: true },
  )
  ...
  function firstIncompleteStepPath(): string {
    const next = registrationSteps.find((step) => !completedSteps.value.includes(step.key))
